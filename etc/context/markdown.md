...
To format Markdown files, use the command `prettier --prose-wrap always --write <file(s)>`. This
command will edit files in place.

If `prettier` is not installed globally, you can run it via
`npx -y prettier@latest --prose-wrap always --write <file(s)>`. If `npx` is not available, you can
