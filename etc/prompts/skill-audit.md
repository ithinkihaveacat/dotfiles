...
- **Skills** (`etc/skills/*/SKILL.md`): The actual skill files used by agents.
  These are the primary deliverable.

- **Prompts** (`etc/prompts/*-skill.md`): Instructions that were used to
  generate each skill initially. A capable agent following a prompt should
  produce a skill very similar to the current one on disk. Prompts encode
  decisions and conventions that may not be obvious from the specification
  alone.
...
Review and update all skills in `etc/skills/` to ensure:
...
For each skill in `etc/skills/`:

1. Read the `SKILL.md` file
2. List all scripts in the `scripts/` subdirectory
3. Read the corresponding prompt in `etc/prompts/<skill-name>-skill.md`
...
ls -la etc/skills/adb/scripts/
```

If a script is referenced but doesn't exist, either:

- Add the missing symlink (if the script exists in `bin/`)
...
ls etc/skills/adb/scripts/ | sort > /tmp/scripts.txt
grep -oE 'scripts/[a-z0-9-]+' etc/skills/adb/SKILL.md \
...
- `bin/adb-*` and `bin/wearableservice-*` → adb skill
- `bin/jetpack` subcommands → jetpack skill
- AI analysis scripts → ai-analysis skill
- `bin/emumanager` subcommands → emumanager skill

For each new script found:

1. Symlink it into the appropriate `scripts/` directory
2. Add documentation to `SKILL.md` and `references/command-index.md`
3. Update the generating prompt if needed
...
```bash
# Check script help
etc/skills/adb/scripts/adb-screenshot --help
```
...
The prompts in `etc/prompts/` serve two purposes:
...
- New script added to `bin/` but not documented in skill
...
- Script removed from `bin/` but still documented
...
- [ ] All skills in `etc/skills/` reviewed
