...
Produce a self-contained skill directory at `skills/emumanager/` that an agent
can use to:
...
Also verify the `bin/emumanager` symlink entrypoint resolves to the skill
script.
...
├── scripts/              # Canonical script implementation
...
Ensure `scripts/emumanager` exists as an executable file (canonical source).
Also ensure `bin/emumanager` is a symlink to
`../skills/emumanager/scripts/emumanager`. Preserve filename and executable
permissions.

### Transitive Script Dependencies

The `emumanager` script is self-contained. No additional script files are
required.
...
- Example:
  `compatibility: Requires Java 17+, curl, and unzip. Hardware acceleration`
  `(KVM on Linux, HVF on macOS) is required for emulators. Needs network`
  `access for SDK downloads. Designed for filesystem-based agents with bash`
  `access.`
...
- [ ] Skill directory exists at `skills/emumanager/`
- [ ] `scripts/emumanager` is an executable file
- [ ] `bin/emumanager` symlinks to `../skills/emumanager/scripts/emumanager`
...
- Implement `scripts/emumanager` as a regular file (not a symlink)
- Create/maintain the `bin/emumanager` symlink:
  `ln -s ../skills/emumanager/scripts/emumanager bin/emumanager`
- Verify executable bits with `chmod +x scripts/*` if needed
- Treat `skills/emumanager/scripts/emumanager` as source of truth
- Test both `scripts/emumanager` and `bin/emumanager`
