...
Produce a self-contained skill directory at `skills/adb/` that an agent can use
to:
...
2. **Examine relevant scripts in `skills/adb/scripts/`**:
   ...
   - All `wearableservice-*` scripts (4 scripts for Wear OS data layer
     inspection)
   - `packagename` (unified package management with ~20 subcommands)
   - Relevant `apk-*` scripts (~16 scripts for APK analysis)

   Also verify corresponding `bin/*` symlink entrypoints for scripts that should
...
├── scripts/              # Canonical script implementations
...
Ensure these executable files exist in `scripts/` as canonical implementations:

**Required (core functionality):**

- All `scripts/adb-*` scripts
...
- Example:
  `compatibility: Requires adb. Some scripts require magick (ImageMagick),`
  `aapt, or scrcpy. Designed for filesystem-based agents with bash access.`
...
- **Display/demo mode**: demo on/off, font scale, touches, theme customization
  (Wear OS 5+/API 36+), theme settings
...
- [ ] Skill directory exists at `skills/adb/`
...
- [ ] `scripts/` contains all required executable scripts
- [ ] Matching `bin/` symlinks exist for required scripts
...
- Create/maintain `bin/` symlinks to those scripts (e.g.,
  `ln -s ../skills/adb/scripts/script bin/script`)
- Verify executable bits with `chmod +x scripts/*` if needed
- Treat `skills/adb/scripts/*` as source of truth
