...
2. **Examine `skills/jetpack/scripts/jetpack` thoroughly**:
...
Also verify the `bin/jetpack` symlink entrypoint resolves to the skill script.
...
Ensure `scripts/jetpack` exists as an executable file (canonical source). Also
ensure `bin/jetpack` is a symlink to `../skills/jetpack/scripts/jetpack`.
Preserve filename and executable permissions.

### Transitive Script Dependencies

The `jetpack` script is self-contained. No additional script files are required.
...
4. **search** - Search for artifacts by package or class name (mention
   `--force`)
...
Include a section at the end of `SKILL.md` linking to the files in
`references/`:
...
- [ ] Skill directory exists at `skills/jetpack/`
...
- [ ] `scripts/jetpack` is an executable file
- [ ] `bin/jetpack` symlinks to `../skills/jetpack/scripts/jetpack`
...
- Implement `scripts/jetpack` as a regular file (not a symlink)
- Create/maintain the `bin/jetpack` symlink:
  `ln -s ../skills/jetpack/scripts/jetpack bin/jetpack`
- Verify executable bits with `chmod +x scripts/*` if needed
- Treat `skills/jetpack/scripts/jetpack` as source of truth
- Test both `scripts/jetpack` and `bin/jetpack`
